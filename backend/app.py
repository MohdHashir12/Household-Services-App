...
app.config['MAIL_PASSWORD'] = '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'  
app.config['MAIL_DEFAULT_SENDER'] = '22f3002812@ds.study.iitm.ac.in'  
mail = Mail(app)
...
def send_daily_reminders():
    ...
    for service_request in pending_requests:
      ...
      chat_hook_url = 'https://chat.googleapis.com/v1/spaces/AAQAxohlhfQ/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=ZXaEB09rLP6lQP_9ZMxPFuk0rtVWCfIwTcIJ-TfU0DI'
    ...
    for customer in customers:
      ...
      try:
        ...
        msg = Message(
            ...
            recipients=['22f3002812@ds.study.iitm.ac.in'],
            html=html_content
        )
        mail.send(msg)
...
def render_report_as_html(report):
    ...
    <body>
        <div class="report-container">
            ...
            <div class="report-item">
                <span>Total Spent:</span> INR{report['total_spent']}
            </div>
            <div class="report-item">
...
def notify_admin(message):
    admin_email = "22f3002812@ds.study.iitm.ac.in"  
    msg = Message(
...
def professional_profile():
    def json_response(message, category='success', status=200, data=None):
        response = {
            "message": message,
            "category": category
        }
    ...
    try:
        ...
        if claims['role'] != 'professional':
            return json_response("Not a Professional!!", "danger", 401)
        ...
        if request.method == 'GET':
            ...
            if 'file' in request.files:
                ...
                if file and file.filename != '':
                    if not allowed_file(file.filename):
                        return json_response(
                            "Invalid file format! Please upload only images and PDFs.",
                            "danger",
                            400
                        )
            ...
            return json_response("Professional Profile updated successfully!")
...
def professional_dashboard():
    """
    Retrieve the professional's dashboard data.
    """
    claims = get_jwt()
    if claims['role'] != 'professional':
    ...
    try:
        ...
        service_requests_closed = ServiceRequest.query.filter(
            ServiceRequest.professional_id == professional_id,
            ServiceRequest.service_status != 'requested'
        ).all()

        
        customer_ids = {req.customer_id for req in service_requests + service_requests_closed}
        service_ids = {req.service_id for req in service_requests + service_requests_closed}

        cust_dict = {
            cust.user_id: cust.as_dict()
            for cust in CustomerProfile.query.filter(CustomerProfile.user_id.in_(customer_ids)).all()
        }
        
        service_dict = {
            svc.id: svc.as_dict()
            for svc in Service.query.filter(Service.id.in_(service_ids)).all()
        }
        ...
        return jsonify({
            ...
            "service_requests_closed": [req.as_dict() for req in service_requests_closed],
            "cust_dict": cust_dict,
