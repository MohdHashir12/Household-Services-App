  ...
  methods: {
    async fetchReviewsData() {
      try {
        ...
        const response = await fetch(`/professional/summary/reviews/${userId}`, {
          method: 'GET',
          headers: {
    ...
    async fetchServiceRequests() {
      try {
        ...
        const response = await fetch(`/professional/summary/service_requests/${userId}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
